...
import {
  Search, X, BookOpen, Briefcase, Clock, AlertTriangle,
  ChevronUp, ChevronDown, Github
} from "lucide-react";
...
function ExperienceBadge({ exp }: { exp: string }) {
  ...
  const color =
    years >= 5 ? "bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800" :
    years >= 4 ? "bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800" :
    years >= 3 ? "bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800" :
...
function ProjectsBadge({ count }: { count: string }) {
  ...
  const color =
    num >= 20 ? "bg-purple-50 dark:bg-purple-950/40 text-purple-700 dark:text-purple-400 border-purple-200 dark:border-purple-800" :
    num >= 15 ? "bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-400 border-indigo-200 dark:border-indigo-800" :
    num >= 10 ? "bg-cyan-50 dark:bg-cyan-950/40 text-cyan-700 dark:text-cyan-400 border-cyan-200 dark:border-cyan-800" :
                "bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700";
  return (
    <span className={`inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-md border ${color}`}>
    ...
    <tr className="border-b border-gray-100 dark:border-gray-800">
      {[1,2,3,4,5].map(i => (
        <td key={i} className="px-4 py-3.5">
          <div className="h-4 bg-gray-100 dark:bg-gray-800 rounded-md animate-pulse" style={{ width: `${50 + i * 10}%` }} />
        </td>
      ))}
    </tr>
  );
...
function SortIcon({ col, sortKey, sortDir }: { col: SortKey; sortKey: SortKey; sortDir: SortDir }) {
  if (col !== sortKey) return <ChevronUp className="w-3 h-3 text-gray-300 dark:text-gray-700" strokeWidth={2} />;
  return sortDir === "asc"
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="relative w-64">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 pointer-events-none" strokeWidth={1.8} />
          <input
            type="text"
            value={tableQuery}
      ...
      {!loading && skills.length > 0 && (
        ...
        >
          <span><span className="font-semibold text-gray-900 dark:text-white">{skills.length}</span> skills total</span>
          {tableQuery && (
            <>
              <span><span className="font-semibold text-gray-900 dark:text-white">{filtered.length}</span> matched</span>
              <span className="text-blue-500 dark:text-blue-400">Filtering: "{tableQuery}"</span>
            </>
          )}
        </motion.div>
      )}
      ...
      <div className="flex-1 overflow-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="text-sm text-gray-400 dark:text-gray-600">No skills match "{tableQuery}"</p>
          </div>
        ) : (
          <motion.div
            key={selectedKey}
                ...
                <table className="w-full min-w-[700px]">
                  <thead>
                    <tr className="border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
                      ...
                      <th className={`${thBase} ${sortKey === "totalProjects" ? thActive : thInactive}`} onClick={() => handleSort("totalProjects")}>
                        <div className="flex items-center gap-1.5">
                      ...
                      <th className={`${thBase} ${sortKey === "totalExperience" ? thActive : thInactive}`} onClick={() => handleSort("totalExperience")}>
                        <div className="flex items-center gap-1.5">
                  ...
                  <tbody>
                    {sorted.map((skill, i) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3.5">
                          <ProjectsBadge count={skill.totalProjects} />
                        </td>

                        <td className="px-4 py-3.5">
                          <ExperienceBadge exp={skill.totalExperience} />
                        </td>
