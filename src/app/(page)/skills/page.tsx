        ...
        <AnimatePresence>
          {!collapsed && (
            <motion.span
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="text-[11px] font-bold text-gray-900 dark:text-white uppercase tracking-widest"
            >
              Categories
            </motion.span>
          )}
...
export default function SkillsPage() {
  ...
  const categories = Array.from(new Set(allSkills.map((s) => s.skillCategory)));

  const skillsByCategory: Record<string, Skill[]> = {};
  for (const cat of categories) {
    skillsByCategory[cat] = allSkills.filter((s) => s.skillCategory === cat);
  }
