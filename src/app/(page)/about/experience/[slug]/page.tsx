...
const SLUG_TO_FILTER: Record<string, (e: Experience) => boolean> = {
  all: () => true,
  "full-stack-development": (e) =>
    e.skills.some((s) => s.toLowerCase().includes("full stack")),
  sdet: (e) =>
    e.skills.some((s) => s.toLowerCase().includes("sdet")) ||
    e.role.toLowerCase().includes("sdet"),
  "product-design": (e) =>
    e.skills.some((s) => s.toLowerCase().includes("product design")),
  "team-leadership": (e) =>
    e.skills.some((s) => s.toLowerCase().includes("team leadership")),
  current: (e) => e.duration.end === "Present",
};
  ...
  return (
    <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-6 space-y-5">
      ...
      <div className="flex gap-2 flex-wrap pt-1">
        {[80, 100, 70, 90, 60].map((_, i) => (
          <SkeletonPulse key={i} className="h-6 rounded-lg w-20" />
        ))}
      </div>
    </div>
  );
...
function AchievementCard({ achievement, index }: { achievement: Achievement; index: number }) {
  ...
  const colors = [
    { bg: "bg-amber-50 dark:bg-amber-950/30", border: "border-amber-200 dark:border-amber-800", icon: "text-amber-600 dark:text-amber-400", iconBg: "bg-amber-100 dark:bg-amber-900/50" },
    { bg: "bg-blue-50 dark:bg-blue-950/30", border: "border-blue-200 dark:border-blue-800", icon: "text-blue-600 dark:text-blue-400", iconBg: "bg-blue-100 dark:bg-blue-900/50" },
    { bg: "bg-purple-50 dark:bg-purple-950/30", border: "border-purple-200 dark:border-purple-800", icon: "text-purple-600 dark:text-purple-400", iconBg: "bg-purple-100 dark:bg-purple-900/50" },
    { bg: "bg-emerald-50 dark:bg-emerald-950/30", border: "border-emerald-200 dark:border-emerald-800", icon: "text-emerald-600 dark:text-emerald-400", iconBg: "bg-emerald-100 dark:bg-emerald-900/50" },
  ];
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="text-xs font-bold text-gray-900 dark:text-white mb-1">{achievement.title}</p>
          <p className="text-[11px] text-gray-500 dark:text-gray-400 leading-relaxed">{achievement.description}</p>
        </div>
      </div>
    </motion.div>
  );
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-1">{exp.company}</p>
        ...
        {expanded && (
          ...
          >
            <div className="px-6 pb-6 pt-1 space-y-5 border-t border-gray-100 dark:border-gray-800">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="pt-4">
                <div className="flex items-center gap-2 mb-2.5">
                  <Layers className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500" strokeWidth={1.8} />
                  <p className="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">About This Role</p>
                </div>
                <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{exp.about}</p>
              </motion.div>

              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }}>
                <div className="flex items-center gap-2 mb-3">
                  <CheckCircle2 className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500" strokeWidth={1.8} />
                  <p className="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Highlights</p>
                </div>
              ...
              {exp.achievements.length > 0 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }}>
                  <div className="flex items-center gap-2 mb-3">
                    <Trophy className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500" strokeWidth={1.8} />
                    <p className="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Key Achievements</p>
                  </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35 }}>
                  <div className="flex items-center gap-2 mb-3">
                    <Star className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500" strokeWidth={1.8} />
                    <p className="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Skills</p>
                  </div>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
                  <div className="flex items-center gap-2 mb-3">
                    <Code2 className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500" strokeWidth={1.8} />
                    <p className="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Tech Stack</p>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
...
export default function ExperienceSlugPage() {
  ...
  const label = SLUG_TO_LABEL[slug] ?? "All Experience";
  const filterFn = SLUG_TO_FILTER[slug] ?? (() => true);

  const [allData, setAllData] = useState<Experience[]>([]);
  ...
  const data = allData.filter(filterFn);
  ...
  return (
    <div className="flex-1 flex flex-col h-[calc(100vh-56px)] bg-white dark:bg-gray-950 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 py-8">
          ...
          {error && !loading && (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <div className="text-center">
                <p className="text-sm font-medium text-gray-800 dark:text-gray-200 mb-1">{error}</p>
                <p className="text-xs text-gray-400 dark:text-gray-600">
                  Check that experience.data.json exists in the public folder.
                </p>
              </div>
            </div>
          )}
          ...
          {!loading && !error && data.length > 0 && (
            ...
            >
              ...
              >
                {[
                  { icon: Briefcase, label: "Roles", value: String(data.length) },
                  { icon: Users, label: "Active Now", value: String(currentRoles) },
                  { icon: Code2, label: "Technologies", value: String(totalTech) },
                  { icon: Star, label: "Skills", value: String(totalSkills) },
                ].map(({ icon: Icon, label: statLabel, value }, i) => (
                  <div key={i} className="flex items-center gap-2">
                    {i > 0 && <div className="w-px h-8 bg-gray-200 dark:bg-gray-800 mr-2" />}
                    <div className="w-8 h-8 rounded-lg bg-gray-900 dark:bg-white flex items-center justify-center shrink-0">
                      <Icon className="w-4 h-4 text-white dark:text-gray-900" strokeWidth={1.8} />
                    </div>
                    <div>
                      <p className="text-xs font-bold text-gray-900 dark:text-white leading-none">{value}</p>
                      <p className="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wide mt-0.5">{statLabel}</p>
                    </div>
                  </div>
                ))}
              </motion.div>
          ...
          {!loading && !error && data.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              <Briefcase className="w-8 h-8 text-gray-300 dark:text-gray-700" strokeWidth={1.5} />
              <p className="text-sm text-gray-400 dark:text-gray-600">
                No experience entries found for &quot;{label}&quot;.
              </p>
            </div>
          )}
