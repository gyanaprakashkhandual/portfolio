...
const CATEGORIES = [
  ...
  { id: "full-stack-development", label: "Full Stack Development", icon: <Code2 size={15} /> },
  { id: "sdet", label: "SDET", icon: <FlaskConical size={15} /> },
  { id: "product-design", label: "Product Design", icon: <Star size={15} /> },
  { id: "team-leadership", label: "Team Leadership", icon: <Users size={15} /> },
  { id: "current", label: "Current Roles", icon: <Briefcase size={15} /> },
];
...
export default function ExperienceSidebar() {
  ...
  const sidebarItems: SidebarItem[] = CATEGORIES.map((cat) => ({
    ...
    label: cat.label,
    icon: cat.icon,
    onClick: () => router.push(`/about/experience/${cat.id}`),
  }));
  ...
  return (
    <div className="sticky top-14 h-[calc(100vh-56px)] flex flex-col bg-white border-r border-gray-100">
      <Sidebar
        ...
        onActiveChange={(id) => {
          router.push(`/about/experience/${id}`);
        }}
      />
    </div>
  );
