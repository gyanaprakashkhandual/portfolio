...
const CATEGORIES = [
  ...
  { id: "social-engineering", label: "Social Engineering", icon: <Star size={15} /> }
];
...
export default function CertificationSidebar() {
  ...
  const sidebarItems: SidebarItem[] = CATEGORIES.map((cat) => ({
    ...
    label: cat.label,
    icon: cat.icon,
    onClick: () => router.push(`/about/certifications/${cat.id}`),
  }));
  ...
  return (
    <div className="sticky top-14 h-[calc(100vh-56px)] flex flex-col bg-white border-r border-gray-100">
      <Sidebar
        ...
        onActiveChange={(id) => {
          router.push(`/about/certifications/${id}`);
        }}
      />
    </div>
  );
