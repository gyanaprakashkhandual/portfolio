...
interface SkillsSidebarProps {
  onSelect: (skills: Skill[], label: string, loading: boolean) => void;
  selectedKey: string;
...
export default function SkillsSidebar({
  ...
  async function fetchSkills(key: string, urlSuffix: string) {
    setLoadingKey(key);
    onKeyChange(key);
    onSelect([], key, true);
    try {
      const url = urlSuffix.includes("/")
        ? `${BASE_URL}/category/${urlSuffix}`
        : `${BASE_URL}/category/${urlSuffix}`;
      const res = await fetch(url);
      const json = await res.json();
      onSelect(json.data ?? [], key, false);
    } catch {
      onSelect([], key, false);
    } finally {
      setLoadingKey(null);
