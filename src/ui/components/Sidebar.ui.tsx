...
const variantStyles: Record<SidebarVariant, string> = {
  ...
  filled:
    "bg-gray-50 dark:bg-gray-800/80 border-r border-gray-200 dark:border-gray-700",
  ghost: "bg-transparent border-r border-gray-100 dark:border-gray-800/60",
  glass:
    "bg-white/70 dark:bg-gray-900/70 backdrop-blur-md border-r border-gray-200/60 dark:border-gray-700/60",
};
...
function SidebarRow({
  ...
  const { setActiveItem, toggleExpand, isExpanded, isActive } =
    useSidebarContext();
  ...
  return (
    <>
      <button
        ...
        `}
        style={{
          paddingLeft: !collapsed && depth > 0 ? indent + 10 : undefined,
        }}
      >
        {item.icon && (
          <span
        ...
        {!collapsed && (
          <>
            ...
            {item.badge !== undefined && (
              <span
                className={`shrink-0 ${s.badgeText} font-semibold px-1.5 py-0.5 rounded-full ${active && !hasChildren ? "bg-white/20 text-white dark:bg-black/20 dark:text-gray-900" : "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400"}`}
              >
                {item.badge}
              </span>
            )}
    ...
    list.map((item, idx) => (
      <React.Fragment key={item.id}>
        ...
        {item.header && !collapsed && (
          <div
            className={`${s.headerPx} pt-3 pb-1 ${s.headerText} font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 select-none`}
          >
            {item.header}
          </div>
        )}
  ...
  if (isSearching && !collapsed) {
    return (
      <div className={`flex flex-col ${s.gap}`}>
        {filtered!.length === 0 ? (
          <div
            className={`${s.px} ${s.py} ${s.labelText} text-gray-400 dark:text-gray-500 text-center py-6`}
          >
            No results
          </div>
        ) : (
...
function SidebarInner({
  ...
  const SidebarContent = (
    <div className={`flex flex-col h-full overflow-hidden ${innerClassName}`}>
      <div
        className={`shrink-0 flex items-center justify-between ${s.headerPx} ${s.headerPy} border-b border-gray-100 dark:border-gray-800 ${headerClassName}`}
      >
        {!collapsed && (
          <div className="flex items-center gap-2 min-w-0">
            ...
            {header ? (
              ...
              <span
                className={`${s.headerTitleText} font-bold text-gray-900 dark:text-white truncate`}
              >
                {title}
              </span>
            ) : null}
        ...
        {showToggle && (
          <Tooltip content="Toggle sidebar (Ctrl+B)" position="left">
            <button
              type="button"
              onClick={cycleMode}
      ...
      {showSearch && !collapsed && (
        <div
          className={`shrink-0 ${s.searchPx} py-2 border-b border-gray-100 dark:border-gray-800`}
        >
          <div
            className={`flex items-center gap-2 ${s.searchHeight} px-2.5 rounded-lg bg-gray-100 dark:bg-gray-800`}
          >
            <Search
              size={s.iconSize - 3}
      ...
      {showSearch && collapsed && (
        <div
          className={`shrink-0 flex justify-center py-2 border-b border-gray-100 dark:border-gray-800`}
        >
          <button
            type="button"
            onClick={() => {
      ...
      {topContent && !collapsed && (
        <div
          className={`shrink-0 ${s.headerPx} py-2 border-b border-gray-100 dark:border-gray-800`}
        >
          {topContent}
        </div>
      )}
      ...
      {bottomContent && !collapsed && (
        <div
          className={`shrink-0 ${s.footerPx} py-2 border-t border-gray-100 dark:border-gray-800`}
        >
          {bottomContent}
        </div>
      )}
      ...
      {footer && (
        <div
          className={`shrink-0 ${s.footerPx} ${s.footerPy} border-t border-gray-100 dark:border-gray-800 ${footerClassName}`}
        >
            ...
            {state.mobileOpen && (
              ...
              >
                <div className="flex flex-col h-full">
                  <div
                    className={`shrink-0 flex items-center justify-between ${s.headerPx} ${s.headerPy} border-b border-gray-100 dark:border-gray-800`}
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      {logo && (
                        <span
                  ...
                  {footer && (
                    <div
                      className={`shrink-0 ${s.footerPx} ${s.footerPy} border-t border-gray-100 dark:border-gray-800`}
                    >
                      {footer}
                    </div>
                  )}
